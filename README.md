...
このアプリケーションは対話型ターミナル（TTY）環境が必要です。
Claude CodeやCI/CD環境など、TTYをサポートしない環境では通常モードでの実行はできません。

### デモモード（非TTY環境向け）

TTY環境が利用できない場合は、デモモードを使用してスライドの内容を静的に出力できます：
